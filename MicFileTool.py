...
def plot_square_mic(SquareMic,squareMicData, minHitRatio,coords, misor_thresh):
    ...
    ax.imshow(img,origin='lower',extent=[minX,maxX,minY,maxY])#, alpha = 0)


    #time for da borders
    import VoxelBorders

    border_list, outside_edges = VoxelBorders.make_square_borders(smdCopy)
    border_lines = []
    edges = []
    for border in border_list:
        x1 = border[0][0][0] *1000 #to adjust for scale
        y1 = border[0][0][1] *1000
        x2 = border[0][1][0] *1000
        y2 = border[0][1][1] *1000
        line = [[x1,y1], [x2,y2]]
